...
export default function DiaryFlowPage() {
  ...
  async function loadDiary() {
    setLoading(true);
    try {
      // Load instrument directly — works without participant login for testing
      const { data: itemData } = await supabase
        .from('instrument_items')
        .select('*')
        .eq('instrument_version_id', 'b1000000-0000-0000-0000-000000000001')
      ...
      // Try to get participant if logged in
  ...
  async function handleSubmit() {
    ...
    const entry = {
      ...
      instrument_version_id: currentItem?.instrument_version_id,
    ...
    try {
      ...
      // Still show submitted for demo purposes
      setSubmitted(true);
    }
    setSubmitting(false);
  }
  ...
  if (submitted) {
    return (
      <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui, sans-serif', padding: '24px', maxWidth: '480px', margin: '0 auto' }}>
        ...
        <div style={{ fontSize: '14px', color: C.textSec, textAlign: 'center', marginBottom: '20px' }}>{t('submittedSub')}</div>
        {!isOnline && (
        ...
        <button
          ...
          style={{ background: C.orange, color: '#fff', border: 'none', borderRadius: '10px', padding: '13px 28px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', width: '100%', maxWidth: '320px' }}
        >
          {t('backToDashboard')}
        </button>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
          ...
          <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(11, 1fr)', gap: '6px' }}>
                  {Array.from({ length: 11 }, (_, i) => (
                    <button key={i} onClick={() => setCurrentValue(i)} style={{ aspectRatio: '1', borderRadius: '10px', border: `2px solid ${currentValue === i ? C.orange : C.border}`, background: currentValue === i ? C.orange : C.bgCard, color: currentValue === i ? '#fff' : i >= 7 ? C.red : C.textPrimary, fontSize: '14px', fontWeight: currentValue === i ? 700 : 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
                      {i}
                    </button>
                  ))}
                </div>
            ...
            {currentItem.item_type === 'yes_no' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[true, false].map(val => (
                  <button key={String(val)} onClick={() => setCurrentValue(val)} style={{ padding: '18px', borderRadius: '14px', border: `2px solid ${currentValue === val ? C.orange : C.border}`, background: currentValue === val ? C.orangeLight : C.bgCard, color: currentValue === val ? C.orange : C.textPrimary, fontSize: '16px', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
            ...
            {currentItem.item_type === 'likert' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[1, 2, 3, 4, 5].map((val, i) => {
                  ...
                  return (
                    <button key={val} onClick={() => setCurrentValue(val)} style={{ padding: '14px 18px', borderRadius: '12px', border: `2px solid ${currentValue === val ? C.orange : C.border}`, background: currentValue === val ? C.orangeLight : C.bgCard, color: currentValue === val ? C.orange : C.textPrimary, fontSize: '14px', fontWeight: currentValue === val ? 600 : 400, cursor: 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: '12px', textAlign: 'left' as const }}>
            ...
            {currentItem.item_type === 'multiple_choice' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                {(currentItem.options_json || []).map((opt: string, i: number) => {
                  ...
                  return (
                    <button key={i} onClick={() => { const prev = Array.isArray(currentValue) ? currentValue : []; setCurrentValue(prev.includes(opt) ? prev.filter((v: string) => v !== opt) : [...prev, opt]); }} style={{ padding: '14px 18px', borderRadius: '12px', border: `2px solid ${selected ? C.orange : C.border}`, background: selected ? C.orangeLight : C.bgCard, color: selected ? C.orange : C.textPrimary, fontSize: '14px', fontWeight: selected ? 600 : 400, cursor: 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: '12px', textAlign: 'left' as const }}>
            ...
            {currentItem.item_type === 'numeric' && (
              <div>
                <input type="number" value={currentValue ?? ''} min={currentItem.min_value ?? undefined} max={currentItem.max_value ?? undefined} onChange={e => setCurrentValue(e.target.value === '' ? null : Number(e.target.value))} style={{ width: '100%', fontSize: '28px', fontWeight: 700, textAlign: 'center', padding: '20px', borderRadius: '14px', border: `2px solid ${C.border}`, color: C.textPrimary, background: C.bgCard, outline: 'none' }} placeholder="0" />
              </div>
            )}

            {/* FREE TEXT */}
            {currentItem.item_type === 'free_text' && (
              <textarea value={currentValue ?? ''} onChange={e => setCurrentValue(e.target.value)} placeholder={t('type_answer')} rows={5} style={{ width: '100%', fontSize: '15px', padding: '16px', borderRadius: '14px', border: `2px solid ${C.border}`, color: C.textPrimary, background: C.bgCard, outline: 'none', resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.6 }} />
            )}

            {error && (
              <div style={{ marginTop: '12px', fontSize: '13px', color: C.red, padding: '10px 14px', background: C.redLight, borderRadius: '8px' }}>{error}</div>
            )}
          </>
        )}
      </div>
      ...
      <div style={{ padding: '16px 20px 32px', background: C.bgCard, borderTop: `1px solid ${C.border}`, display: 'flex', gap: '10px' }}>
        {currentIndex > 0 && (
          <button onClick={handleBack} style={{ flex: 1, padding: '14px', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.bgCard, color: C.textSec, fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>
            {t('back')}
          </button>
        )}
        {!currentItem?.required && !isLast && (
          <button onClick={handleSkip} style={{ padding: '14px 20px', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.bgCard, color: C.textMuted, fontSize: '14px', cursor: 'pointer' }}>
            {t('skip')}
          </button>
        )}
        <button onClick={isLast ? handleSubmit : handleNext} disabled={submitting} style={{ flex: 2, padding: '14px', borderRadius: '12px', border: 'none', background: C.orange, color: '#fff', fontSize: '14px', fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1, transition: 'all 0.15s' }}>
