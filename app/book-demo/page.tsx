...
export default function BookDemoPage() {
  ...
  useEffect(() => {
    ...
    const docs = [
      { name: 'Protocol v2.1', status: 'Approved', color: '#10B981' },
      { name: 'IRB Decision Letter', status: 'Review', color: '#3B82F6' },
      { name: 'Monitoring Plan', status: 'Approved', color: '#10B981' },
      { name: 'SAE Report Q2', status: 'Draft', color: '#F59E0B' },
      { name: 'Informed Consent v3', status: 'Approved', color: '#10B981' },
      { name: 'Site Signature Sheet', status: 'Review', color: '#3B82F6' },
      { name: 'IP Shipment Docs', status: 'Approved', color: '#10B981' },
      { name: 'Audit Certificate', status: 'Approved', color: '#10B981' },
      { name: 'Financial Disclosure', status: 'Draft', color: '#F59E0B' },
      { name: 'Pregnancy Report', status: 'Review', color: '#3B82F6' },
      { name: 'Randomization Plan', status: 'Approved', color: '#10B981' },
      { name: 'Clinical Trial Agreement', status: 'Approved', color: '#10B981' },
    ];
    let docIndex = 0;
    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        :root{--orange:#F97316;--orange-light:#FFF7ED;--orange-dark:#EA580C;--green:#10B981;--green-light:#ECFDF5;--blue:#3B82F6;--red:#EF4444;--purple:#8B5CF6;--amber:#F59E0B;--pink:#EC4899;--indigo:#6366F1;--text:#111827;--text-sec:#374151;--text-muted:#6B7280;--bg:#FFFFFF;--bg-sec:#F9FAFB;--bg-tert:#F3F4F6;--border:#E5E7EB;}
        ...
        .ns-item{display:flex;align-items:center;gap:9px;font-size:12px;color:var(--text-sec);margin-bottom:8px;}
        ...
        .dash-logo{font-size:12px;font-weight:500;}
        ...
        .mac-dots{display:flex;gap:4px;margin-left:6px;}
        .mac-dot{width:6px;height:6px;border-radius:50%;}
        ...
        .stat-val{font-size:17px;font-weight:500;line-height:1;}
        .stat-lbl{font-size:8px;color:var(--text-muted);margin-top:2px;}
        ...
        .doc-item{display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:0.5px solid var(--bg-tert);}
        .doc-item:last-child{border-bottom:none;}
        .doc-item.new-doc{animation:newDoc .35s ease;}
        .doc-dot{width:5px;height:5px;border-radius:50%;flex-shrink:0;}
        .doc-name{font-size:9px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
        .doc-status{font-size:8.5px;white-space:nowrap;}
        ...
        .zone-row{display:flex;align-items:center;gap:5px;margin-bottom:4px;}
        ...
        <div className="main-grid">
          <div className="left-panel">
            ...
            <div className="left-inner">
              ...
              <div className="feature-list">
                {[
                  { title: 'AI document classification', sub: 'Auto-file via Trinity AI', bg: '#FFF7ED', icon: 'orange' },
                  { title: '21 CFR Part 11 audit trail', sub: 'Electronic signatures', bg: '#ECFDF5', icon: 'green' },
                  { title: 'Inspection readiness score', sub: 'Real-time gap analysis', bg: '#EFF6FF', icon: 'blue' },
                  { title: 'Multi-user team management', sub: 'Roles, permissions, notifications', bg: '#FAF5FF', icon: 'purple' },
                  { title: 'Inspection package export', sub: 'PDF, Excel, Word reports', bg: '#FFFBEB', icon: 'amber' },
                  { title: 'TMF Auditor', sub: 'Document review and approval', bg: '#FEF2F2', icon: 'red' },
                  { title: 'Messages', sub: 'Group and personalised messaging', bg: '#EFF6FF', icon: 'blue' },
                ].map((f, i) => (
                  <div key={i} className="fcard selected" onClick={(e) => (window as any).__toggleFeature(e.currentTarget)} style={{ animation: `fcardIn .4s ease ${.12 + i * .05}s both` }}>
                    <div className="ficon" style={{ background: f.bg }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={f.icon === 'orange' ? '#F97316' : f.icon === 'green' ? '#10B981' : f.icon === 'blue' ? '#3B82F6' : f.icon === 'purple' ? '#8B5CF6' : f.icon === 'amber' ? '#F59E0B' : '#EF4444'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    </div>
                    <div><div className="fcard-title">{f.title}</div><div className="fcard-sub">{f.sub}</div></div>
                  </div>
                ))}
              </div>
          ...
          <div className="right-panel">
            <div className="step-indicators">
              <div className="ind-item" id="ind-0">
                <div className="ind-circle" style={{ background: 'var(--orange)', color: '#fff' }}>1</div>
                <span className="ind-label" style={{ color: 'var(--orange)' }}>About you</span>
              </div>
              <div className="ind-line"><div className="ind-line-fill" id="line-0"></div></div>
              <div className="ind-item" id="ind-1">
                <div className="ind-circle" style={{ background: 'var(--bg-tert)', border: '0.5px solid var(--border)', color: 'var(--text-muted)' }}>2</div>
                <span className="ind-label" style={{ color: 'var(--text-muted)' }}>Your trial</span>
              </div>
              <div className="ind-line"><div className="ind-line-fill" id="line-1"></div></div>
              <div className="ind-item" id="ind-2">
                <div className="ind-circle" style={{ background: 'var(--bg-tert)', border: '0.5px solid var(--border)', color: 'var(--text-muted)' }}>3</div>
                <span className="ind-label" style={{ color: 'var(--text-muted)' }}>Schedule</span>
              </div>
            </div>
            ...
            <div className="step active" id="step-0">
              ...
              <div className="field-row">
                <div><label className="lbl">Full name</label><input className="field" placeholder="Jane Smith" value={formData.fullName} onChange={e => setFormData(p => ({...p, fullName: e.target.value}))}/></div>
                <div><label className="lbl">Work email</label><input className="field" type="email" placeholder="jane@company.com" value={formData.email} onChange={e => setFormData(p => ({...p, email: e.target.value}))}/></div>
              </div>
              <div><label className="lbl">Organisation</label><input className="field" placeholder="Acme Pharma Ltd." value={formData.organisation} onChange={e => setFormData(p => ({...p, organisation: e.target.value}))}/></div>
              <div>
            ...
            <div className="step" id="step-1">
              ...
              <div><label className="lbl">What are you looking to solve?</label><textarea className="field" placeholder="e.g. We need a compliant eTMF for an upcoming Phase II trial starting Q3..." value={formData.message} onChange={e => setFormData(p => ({...p, message: e.target.value}))}></textarea></div>
            ...
            <div className="step" id="step-3">
              <div className="success-wrap">
                ...
                <div style={{ animation: 'fadeUp .4s ease .2s both' }}>
                  ...
                  <p className="success-sub">We will confirm your demo within 24 hours. Check your inbox for a calendar invite.</p>
                </div>
