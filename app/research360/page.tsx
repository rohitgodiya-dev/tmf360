...
export default function TrinityPage(){
  ...
  function switchStudy(studyId:string){
    ...
    if(s){
      ...
      loadStudyData(studyId,orgId);
      setChatMessages([{role:"ai",text:`Switched to study ${studyId}. I've loaded the TMF data and vault documents for this study.`}]);
    }
  }

  // ---- DERIVED TMF DATA ----
  const activeTMF=tmfConfig.filter(c=>c.type==="artifact").map(c=>({
    z:c.zone_num,zn:c.zone_name||"",s:c.section_num||"",
    a:c.artifact_num,an:c.artifact_name,cl:c.classification||"Core",iso:c.iso_ref||"",
  }));
  ...
  async function uploadVaultDoc(){
    ...
    const{data:inserted}=await supabase.from("study_vault").insert([{
      org_id:orgId,study_id:activeStudy.study_id,
      file_name:vaultFile.name,custom_name:vaultCustomName||vaultFile.name,
      document_type:vaultDocType,file_path:path,
      file_size:vaultFile.size,extracted_text:extractedText,
      uploaded_by:user?.email,is_active:true,
    }]).select();

    if(inserted){
      setVaultDocs(prev=>[inserted[0] as VaultDoc,...prev]);
      setVaultProgress("Done!");
      setChatMessages(prev=>[...prev,{role:"ai",text:`I've read "${vaultCustomName||vaultFile.name}" (${vaultDocType}) and added it to the Study Vault. I'll now use this document as a source of truth when answering questions about ${activeStudy.study_id}.`}]);
  ...
  async function runVaultAnalysis(){
    if(!activeStudy||vaultDocs.length===0){
      alert("Upload at least one document to the Study Vault first.");return;
    }
    setAnalysing(true);
    try{
...
[{"finding_type":"gap"|"version_mismatch"|"missing_required"|"protocol_reference","severity":"Critical"|"Major"|"Minor","title":"short title","detail":"detailed explanation","source_doc":"which vault doc you found this in","artifact_ref":"DIA artifact number if applicable"}]
...
Return ONLY the JSON array, no other text.`;
  ...
  async function generateBriefing(){
    ...
    const prompt=`Generate a daily TMF briefing for study ${activeStudy.study_id}. Return JSON with this structure:
{"summary":"2 sentence overview","priority_actions":[{"action":"string","reason":"string","urgency":"High"|"Medium"|"Low"}],"stats":{"completeness":${donePct},"missing":${missing},"pending":${pending},"expiring":${expiring},"ri":${ri}},"vault_insight":"one insight from the vault documents if available"}
...
Return ONLY valid JSON.`;
    try{
      const res=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:prompt,context:"You are a TMF briefing engine. Return only valid JSON."})});
      const data=await res.json();
      const raw=data.response?.replace(/```json|```/g,"").trim();
  ...
  useEffect(()=>{
    if(panel==="briefing"&&!briefing&&activeStudy)generateBriefing();
  },[panel,activeStudy]);
  ...
  function detectFlagReason(doc:any){
    ...
    if(doc.expiry_date&&new Date(doc.expiry_date)<new Date())return`Document expired — effective document expired on ${doc.expiry_date}.`;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"system-ui,-apple-system,sans-serif",background:P.bg}}>
      ...
      {/* Tabs */}
      <div style={{display:"flex",alignItems:"center",gap:"0",borderBottom:`0.5px solid ${P.border}`,background:P.bg,paddingLeft:"1.5rem",flexShrink:0}}>
        {[
          ...
          <button key={t.id} onClick={()=>setPanel(t.id as any)} className={`trinity-tab${panel===t.id?" active":""}`}>
            <i className={`ti ${t.icon}`} style={{fontSize:"13px",marginRight:"5px",verticalAlign:"-1px"}}/>
            {t.label}
          </button>
        ))}
      ...
      {panel==="chat"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{padding:"8px 1.5rem",display:"flex",gap:"6px",flexWrap:"wrap",borderBottom:`0.5px solid ${P.border}`,background:P.bgSec,flexShrink:0}}>
            {[
              "What's my TMF health?",
              "What is the primary endpoint of this study?",
              "What documents are missing from Zone 3?",
              "Summarise the Protocol",
              "What sites are in this trial?",
              "Review a pending document",
            ].map(q=>(
              <button key={q} onClick={()=>setChatInput(q)} style={{fontSize:"11px",border:`0.5px solid ${P.border}`,borderRadius:"20px",padding:"4px 12px",color:P.textSec,background:P.bg,cursor:"pointer",whiteSpace:"nowrap"}}>
                {q}
              </button>
            ))}
            {!vaultHasProtocol&&(
              <span style={{fontSize:"11px",padding:"4px 12px",borderRadius:"20px",background:"#FFFBEB",color:"#92400E",border:"0.5px solid #FDE68A"}}>
                Upload Protocol to vault for full insights
              </span>
            )}
          </div>
          ...
          <div style={{flex:1,overflowY:"auto",background:`linear-gradient(135deg,${P.lavender} 0%,#F5F6FC 45%,${P.bg} 100%)`,display:"flex",flexDirection:"column",alignItems:"center",padding:"20px 0 8px"}}>
            <div style={{width:"100%",maxWidth:"800px",padding:"0 24px",display:"flex",flexDirection:"column",gap:"16px"}}>
              {chatMessages.map((m,i)=>(
                <div key={i} style={{display:"flex",gap:"10px",justifyContent:m.role==="user"?"flex-end":"flex-start",animation:"fadeUp .3s ease"}}>
                  {m.role==="ai"&&(
                  ...
                  <div style={{maxWidth:"78%",display:"flex",flexDirection:"column",gap:"6px"}}>
                    ...
                    {chatDocAction?.msgIdx===i&&!chatDocAction.disabled&&(
                      <div style={{display:"flex",gap:"8px"}}>
                        <button onClick={()=>{
                          const doc=docs.find(d=>d.id===chatMessages[i].classification?.zoneLine);
                          const pendingDoc=docs.find(d=>d.status==="Under Review");
                    ...
                    {(m as any).classStage==="zone"&&(m as any).pendingClassification&&(
                      <div style={{display:"flex",gap:"8px"}}>
                        <button onClick={()=>{
                          ...
                          setChatMessages(prev=>[...prev,{role:"ai",text:`Zone ${cl.zone_num} - ${cl.zone_name} approved.\n\n📄 ${cl.artifact_num} - ${cl.artifact_name}\n\n${cl.issues?.length>0?"⚠️ Issues:\n"+cl.issues.join("\n"):""}\n\nApprove this artifact?`,pendingClassification:cl,classStage:"artifact"} as any]);
                      ...
                      <div style={{display:"flex",gap:"8px"}}>
                        <button onClick={async()=>{
                          ...
                          try{
                            ...
                            const statusMsg=hasIssues?`⚠️ Filed to Draft due to issues:\n${rejReason}`:` ✅ Filed to Zone ${cl.zone_num} — ${cl.artifact_name}\nStatus: Under Review.`;
        ...
        <div style={{flex:1,overflowY:"auto",padding:"1.5rem"}}>
          <div style={{maxWidth:"800px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            ...
            <div style={{background:"#EFF6FF",border:"0.5px solid #BFDBFE",borderRadius:"10px",padding:"12px 16px",fontSize:"11px",color:"#1E40AF"}}>
              <strong>Recommended uploads:</strong> Protocol (most important), Investigator's Brochure, Statistical Analysis Plan, Monitoring Plan, IRB Decision. Trinity will use these to validate documents and identify study-specific gaps.
            </div>
            ...
            <div style={{background:P.bg,border:`0.5px solid ${P.border}`,borderRadius:"12px",padding:"20px"}}>
              ...
              <div
                ...
                style={{border:`1.5px dashed ${vaultFile?P.primary:P.border}`,borderRadius:"10px",padding:"1.5rem",textAlign:"center",cursor:"pointer",background:vaultFile?P.primaryLight:P.bgSec,marginBottom:"12px"}}
                onDragOver={e=>{e.preventDefault();}}
                onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)setVaultFile(f);}}
              >
                ...
                {vaultFile?(
                  <div>
                    ...
                    <div style={{fontSize:"13px",fontWeight:"500",color:P.primary}}>{vaultFile.name}</div>
                    <div style={{fontSize:"11px",color:P.textTert,marginTop:"2px"}}>{(vaultFile.size/1024).toFixed(0)} KB</div>
                  </div>
                ):(
                  <div>
                    <div style={{fontSize:"24px",marginBottom:"6px"}}>📁</div>
                    <div style={{fontSize:"13px",color:P.textSec}}>Drop a PDF here or click to browse</div>
                    <div style={{fontSize:"11px",color:P.textTert,marginTop:"2px"}}>Supports PDF, DOC, DOCX</div>
                  </div>
                )}
              </div>
                  ...
                  <div key={d.id} style={{background:P.bg,border:`0.5px solid ${P.border}`,borderRadius:"10px",padding:"14px 16px",display:"flex",alignItems:"center",gap:"12px"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:"flex",gap:"8px",marginTop:"3px",alignItems:"center"}}>
                        ...
                        {d.extracted_text?(
                          <span style={{fontSize:"10px",color:P.success}}>✓ Text extracted ({d.extracted_text.length} chars)</span>
                        ):(
                          <span style={{fontSize:"10px",color:P.warning}}>⚠ No text extracted</span>
                        )}
                      </div>
                    </div>
          ...
          <div style={{maxWidth:"900px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
                ...
                <p style={{fontSize:"12px",color:P.textTert,marginTop:"3px"}}>Auto-generated from vault document analysis. Study-specific gaps and issues.</p>
              </div>
            ...
            {vaultDocs.length===0&&(
              <div style={{background:"#FFFBEB",border:"0.5px solid #FDE68A",borderRadius:"10px",padding:"12px 16px",fontSize:"11px",color:"#92400E"}}>
                Upload documents to the Study Vault first, then run analysis to generate findings.
              </div>
            )}

            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px"}}>
              {[
                {label:"Critical",color:P.danger,bg:P.dangerLight,count:findings.filter(f=>f.severity==="Critical"&&f.status==="Open").length},
                {label:"Major",color:P.warning,bg:P.warningLight,count:findings.filter(f=>f.severity==="Major"&&f.status==="Open").length},
                {label:"Minor",color:P.blue,bg:P.blueLight,count:findings.filter(f=>f.severity==="Minor"&&f.status==="Open").length},
              ].map((s,i)=>(
                    ...
                    <div key={sev}>
                      ...
                      {sevFindings.map(f=>(
                        <div key={f.id} style={{background:P.bg,border:`0.5px solid ${f.status==="Open"?SEVERITY_COLOR(f.severity):P.border}`,borderRadius:"10px",padding:"14px 16px",marginBottom:"6px",display:"flex",gap:"12px",alignItems:"flex-start",opacity:f.status==="Resolved"?0.6:1}}>
                          ...
                          {f.status==="Open"&&(
                            <button onClick={()=>resolveFinding(f.id)} style={{fontSize:"11px",padding:"5px 12px",background:P.successLight,color:P.success,border:`0.5px solid #A7F3D0`,borderRadius:"6px",cursor:"pointer",flexShrink:0}}>Resolve</button>
                          )}
                        </div>
                      ))}
                    </div>
          ...
          <div style={{maxWidth:"800px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            ...
            {briefingLoading&&!briefing&&(
              <div style={{textAlign:"center",padding:"3rem",color:P.textTert}}>
                <div style={{fontSize:"13px"}}>Trinity is generating your briefing...</div>
              </div>
            )}
            ...
            {briefing&&(
              <>
                {/* Summary card */}
                <div style={{background:`linear-gradient(135deg,${P.navy||"#0F1E3D"} 0%,#1E3A5F 100%)`,borderRadius:"14px",padding:"20px 24px",color:"#fff"}}>
                  ...
                  {briefing.vault_insight&&(
                    <div style={{marginTop:"12px",padding:"10px 14px",background:"rgba(249,115,22,0.2)",borderRadius:"8px",fontSize:"12px",color:"rgba(255,255,255,0.85)",borderLeft:"3px solid #F97316"}}>
                      <strong>From your vault:</strong> {briefing.vault_insight}
                    </div>
                  )}
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:"10px"}}>
                  {[
                    {val:`${briefing.stats?.completeness||donePct}%`,label:"Completeness",color:P.blue},
                    {val:briefing.stats?.missing||missing,label:"Missing",color:P.danger},
                    {val:briefing.stats?.pending||pending,label:"Pending review",color:P.warning},
                    {val:briefing.stats?.expiring||expiring,label:"Expiring (90d)",color:P.warning},
                    {val:`${briefing.stats?.ri||ri}/100`,label:"Readiness",color:ri>=80?P.success:ri>=50?P.primary:P.danger},
                  ].map((s,i)=>(
