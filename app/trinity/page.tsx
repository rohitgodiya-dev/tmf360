...
export default function TrinityPage(){
  ...
  const[chatMessages,setChatMessages]=useState<ChatMsg[]>([{role:"ai",text:"Hi, I'm Trinity — your AI TMF Specialist. I can read your study vault documents, classify uploaded files, identify missing documents, and answer questions about your trial master file.\n\nUpload your Protocol and key study documents to the Study Vault first, and I'll use them to give you study-specific insights."}]);
...
const[checklistGenerated,setChecklistGenerated]=useState(false);
  ...
  async function loadUserContext(uid:string){
    ...
    if(studyData&&studyData.length>0){setStudies(studyData);const active=savedStudyId?studyData.find((s:any)=>s.study_id===savedStudyId)||studyData[0]:studyData[0];setActiveStudy(active);loadStudyData(active.study_id,data.org_id);}
    setLoading(false);
  }
...
if(checklistData&&checklistData.length>0){setChecklist(checklistData);setChecklistGenerated(true);}
  ...
  async function uploadVaultDoc(){
    ...
    if(inserted){setVaultDocs(prev=>[inserted[0] as VaultDoc,...prev]);setVaultProgress("Done!");setChatMessages(prev=>[...prev,{role:"ai",text:`I've read "${vaultCustomName||vaultFile.name}" (${vaultDocType}) and added it to the Study Vault.`}]);setTimeout(()=>{setVaultFile(null);setVaultCustomName("");setVaultProgress("");},2000);}
  ...
  async function sendChat(){
    ...
    if(activeStudy&&/(health|status|readiness|overview)/.test(lower)&&/(tmf|study|trial)/.test(lower)){setChatMessages(prev=>[...prev,{role:"ai",text:`${donePct}% complete, with ${missing} core documents outstanding and ${pending} awaiting review. Inspection readiness is ${ri}/100 for ${activeStudy.study_id}.`,isHealthCard:true,sourceTags:["Gap analysis","Inspection readiness","Document tracker"]}]);setChatLoading(false);return;}
    if(activeStudy&&/why/.test(lower)&&/(flag|reject)/.test(lower)){const flaggedDoc=docs.find(d=>d.status==="Draft"&&(d as any).rejection_reason);if(flaggedDoc){setChatMessages(prev=>[...prev,{role:"ai",text:`"${flaggedDoc.custom_file_name||flaggedDoc.artifact_name}" was flagged:\n${(flaggedDoc as any).rejection_reason}`,sourceTags:["Document tracker","Audit trail"]}]);}else{setChatMessages(prev=>[...prev,{role:"ai",text:`No flagged documents in ${activeStudy.study_id} right now.`}]);}setChatLoading(false);return;}
  ...
  if(loading)return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:P.lavender,fontFamily:"system-ui,-apple-system,sans-serif"}}><div style={{textAlign:"center"}}><div style={{fontSize:"24px",fontWeight:"700",color:P.text}}>TMF<span style={{color:P.primary}}>360</span></div><div style={{fontSize:"12px",color:P.textTert,marginTop:"4px"}}>Loading Trinity...</div></div></div>);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"system-ui,-apple-system,sans-serif",background:P.bg}}>
      ...
      <style>{`@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}.trinity-tab{border:none;background:transparent;cursor:pointer;padding:10px 16px;font-size:12px;font-weight:500;border-bottom:2px solid transparent;transition:all .2s;font-family:inherit;}.trinity-tab.active{border-bottom-color:#F97316;color:#F97316;}.trinity-tab:not(.active){color:#6B7280;}.trinity-tab:not(.active):hover{color:#374151;}`}</style>
      ...
      <header style={{display:"flex",alignItems:"center",gap:"12px",padding:"0 1.5rem",height:"52px",borderBottom:`0.5px solid ${P.border}`,background:P.bg,flexShrink:0}}>
        <a href="/platform" style={{display:"flex",alignItems:"center",gap:"6px",textDecoration:"none",color:P.textTert,fontSize:"12px"}}><i className="ti ti-arrow-left" style={{fontSize:"14px"}}/>Back to TMF360</a>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          ...
          <div><div style={{fontSize:"14px",fontWeight:"700",color:P.text}}>Trinity <span style={{color:P.primary}}>AI Specialist</span></div><div style={{fontSize:"10px",color:P.textTert}}>Study-aware · Vault-powered · Real-time</div></div>
        </div>
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:"10px"}}>
          {studies.length>0&&(<select value={activeStudy?.study_id||""} onChange={e=>switchStudy(e.target.value)} style={{fontSize:"11px",border:`0.5px solid ${P.border}`,borderRadius:"8px",padding:"5px 10px",background:P.bg,fontFamily:"inherit"}}>{studies.map(s=><option key={s.study_id} value={s.study_id}>{s.study_id}</option>)}</select>)}
      ...
      {panel==="chat"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{padding:"8px 1.5rem",display:"flex",gap:"6px",flexWrap:"wrap",borderBottom:`0.5px solid ${P.border}`,background:P.bgSec,flexShrink:0}}>
            {["What's my TMF health?","What is the primary endpoint of this study?","What documents are missing from Zone 3?","Summarise the Protocol","What sites are in this trial?","Review a pending document"].map(q=>(<button key={q} onClick={()=>setChatInput(q)} style={{fontSize:"11px",border:`0.5px solid ${P.border}`,borderRadius:"20px",padding:"4px 12px",color:P.textSec,background:P.bg,cursor:"pointer",whiteSpace:"nowrap"}}>{q}</button>))}
            ...
            <div style={{width:"100%",maxWidth:"800px",padding:"0 24px",display:"flex",flexDirection:"column",gap:"16px"}}>
              {chatMessages.map((m,i)=>(
                <div key={i} style={{display:"flex",gap:"10px",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                  {m.role==="ai"&&(<span style={{width:"28px",height:"28px",borderRadius:"50%",flexShrink:0,background:`linear-gradient(135deg,${P.primaryLight},#fff)`,border:`0.5px solid ${P.primaryLight}`,display:"flex",alignItems:"center",justifyContent:"center",color:P.primary,marginTop:"2px"}}><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c.5 3.6 2.2 6 6.5 6.5-4.3.5-6 2.9-6.5 6.5-.5-3.6-2.2-6-6.5-6.5C9.8 8 11.5 5.6 12 2Z"/><path d="M19 15c.25 1.6 1 2.3 2.6 2.5-1.6.25-2.3 1-2.6 2.6-.25-1.6-1-2.3-2.6-2.6 1.6-.2 2.3-.9 2.6-2.5Z"/></svg></span>)}
                      ...
                      <div style={{border:`0.5px solid ${P.border}`,borderRadius:"10px",padding:"14px 16px",background:P.bg,display:"flex",flexDirection:"column",gap:"10px"}}>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                          {m.validation.checks?.map((c:any,ci:number)=>(
                            <div key={ci} style={{display:"flex",alignItems:"flex-start",gap:"8px",padding:"7px 10px",borderRadius:"7px",background:c.pass?P.successLight:P.warningLight}}>
                              <span style={{fontSize:"14px",flexShrink:0}}>{c.pass?"✅":"⚠️"}</span>
                        ...
                        <div style={{display:"flex",gap:"8px"}}>
                          <button onClick={async()=>{
                            const cl=m.pendingClassification;
                            setChatMessages(prev=>prev.map((msg,mi)=>mi===i?{...msg,text:"__VALIDATE_DONE__"} as any:msg));
                            await fileDocument(cl,m.validation);
                          }} style={{fontSize:"12px",fontWeight:"600",padding:"6px 16px",background:P.success,color:"#fff",border:"none",borderRadius:"7px",cursor:"pointer"}}>✓ Confirm & File</button>
                          <button onClick={()=>{setChatMessages(prev=>prev.map((msg,mi)=>mi===i?{...msg,text:"__VALIDATE_DONE__"} as any:msg));setChatMessages(prev=>[...prev,{role:"ai",text:"Filing cancelled."}]);}} style={{fontSize:"12px",fontWeight:"600",padding:"6px 16px",background:P.danger,color:"#fff",border:"none",borderRadius:"7px",cursor:"pointer"}}>✗ Cancel</button>
                        </div>
                      </div>
                    )}
                      ...
                      <div style={{display:"flex",gap:"8px"}}>
                        <button onClick={async()=>{
                          ...
                          try{
                            ...
                            setChatMessages(prev=>[...prev,{role:"ai",text:"__VALIDATE__",pendingClassification:cl,validation} as any]);
                          }catch{
                            await fileDocument(cl,null);
                          }
                          setChatLoading(false);
              ...
              {chatLoading&&(<div style={{display:"flex",gap:"10px"}}><span style={{width:"28px",height:"28px",borderRadius:"50%",background:`linear-gradient(135deg,${P.primaryLight},#fff)`,border:`0.5px solid ${P.primaryLight}`,display:"flex",alignItems:"center",justifyContent:"center",color:P.primary,flexShrink:0}}><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c.5 3.6 2.2 6 6.5 6.5-4.3.5-6 2.9-6.5 6.5-.5-3.6-2.2-6-6.5-6.5C9.8 8 11.5 5.6 12 2Z"/></svg></span><div style={{background:P.bg,border:`0.5px solid ${P.border}`,borderRadius:"10px",padding:"10px 14px",display:"flex",gap:"4px",alignItems:"center"}}>{[0,1,2].map(i=><span key={i} style={{width:"5px",height:"5px",borderRadius:"50%",background:P.textTert,display:"inline-block",animation:"bounce 0.9s infinite",animationDelay:`${i*0.15}s`}}/>)}</div></div>)}
              <div ref={messagesEnd}/>
            </div>
          </div>
          ...
          <div style={{display:"flex",justifyContent:"center",padding:"14px 0 18px",background:`linear-gradient(135deg,${P.lavender} 0%,#F5F6FC 45%,${P.bg} 100%)`,flexShrink:0}}>
            ...
            <div style={{width:"100%",maxWidth:"800px",margin:"0 24px",display:"flex",alignItems:"center",gap:"8px",background:P.bg,border:`0.5px solid ${P.border}`,borderRadius:"26px",padding:"6px 8px 6px 14px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
              <button onClick={()=>chatFileInput.current?.click()} style={{width:"32px",height:"32px",borderRadius:"50%",border:"none",background:"transparent",color:P.textTert,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20.5 12.5 12 21a5 5 0 0 1-7-7l9-9a3.5 3.5 0 0 1 5 5l-9 9a2 2 0 0 1-3-3l8-8"/></svg></button>
              ...
              <button onClick={sendChat} disabled={chatLoading} style={{width:"32px",height:"32px",borderRadius:"50%",flexShrink:0,background:chatLoading?P.bgTert:P.primary,border:"none",color:chatLoading?P.textTert:"#fff",display:"flex",alignItems:"center",justifyContent:"center",cursor:chatLoading?"not-allowed":"pointer"}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{maxWidth:"800px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            ...
            <div style={{background:P.bg,border:`0.5px solid ${P.border}`,borderRadius:"12px",padding:"20px"}}>
              ...
              <div onClick={()=>vaultFileInput.current?.click()} style={{border:`1.5px dashed ${vaultFile?P.primary:P.border}`,borderRadius:"10px",padding:"1.5rem",textAlign:"center",cursor:"pointer",background:vaultFile?P.primaryLight:P.bgSec,marginBottom:"12px"}} onDragOver={e=>{e.preventDefault();}} onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)setVaultFile(f);}}>
                ...
                {vaultFile?(<div><div style={{fontSize:"24px",marginBottom:"6px"}}>📄</div><div style={{fontSize:"13px",fontWeight:"500",color:P.primary}}>{vaultFile.name}</div><div style={{fontSize:"11px",color:P.textTert,marginTop:"2px"}}>{(vaultFile.size/1024).toFixed(0)} KB</div></div>):(<div><div style={{fontSize:"24px",marginBottom:"6px"}}>📁</div><div style={{fontSize:"13px",color:P.textSec}}>Drop a PDF here or click to browse</div><div style={{fontSize:"11px",color:P.textTert,marginTop:"2px"}}>Supports PDF, DOC, DOCX</div></div>)}
              </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"1.5rem"}}>
          <div style={{maxWidth:"800px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <button onClick={generateBriefing} disabled={briefingLoading} style={{fontSize:"12px",padding:"8px 16px",background:P.primary,color:"#fff",border:"none",borderRadius:"8px",cursor:"pointer",display:"flex",alignItems:"center",gap:"6px",opacity:briefingLoading?0.6:1}}>{briefingLoading?<i className="ti ti-loader" style={{fontSize:"13px",animation:"spin 1s linear infinite"}}/>:<i className="ti ti-refresh" style={{fontSize:"13px"}}/>}{briefingLoading?"Generating...":"Refresh"}</button>
            </div>
            ...
            {briefing&&(
              <>
                ...
                <div style={{textAlign:"center",fontSize:"11px",color:P.textTert}}>Briefing generated by Trinity at {new Date().toLocaleTimeString()} · Based on live TMF data{vaultDocs.length>0?` + ${vaultDocs.length} vault document${vaultDocs.length!==1?"s":""}`:""}</div>
              </>
            )}
          </div>
        </div>
