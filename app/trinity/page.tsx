...
const IconStar=()=><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c.5 3.6 2.2 6 6.5 6.5-4.3.5-6 2.9-6.5 6.5-.5-3.6-2.2-6-6.5-6.5C9.8 8 11.5 5.6 12 2Z"/><path d="M19 15c.25 1.6 1 2.3 2.6 2.5-1.6.25-2.3 1-2.6 2.6-.25-1.6-1-2.3-2.6-2.6 1.6-.2 2.3-.9 2.6-2.5Z"/></svg>;
...
const IconUpArrow=()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg>;
const IconPaperclip=()=><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20.5 12.5 12 21a5 5 0 0 1-7-7l9-9a3.5 3.5 0 0 1 5 5l-9 9a2 2 0 0 1-3-3l8-8"/></svg>;
const IconCircleCheck=()=><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="9 12 11 14 15 10"/></svg>;
...
export default function TrinityPage(){
  ...
  async function loadUserContext(uid:string){
    ...
    if(studyData&&studyData.length>0){
      ...
      loadStudyData(active.study_id,data.org_id);
    }
    setLoading(false);
  }
  ...
  async function uploadVaultDoc(){
    ...
    if(inserted){
      ...
      setChatMessages(prev=>[...prev,{role:"ai",text:`I've read "${vaultCustomName||vaultFile.name}" (${vaultDocType}) and added it to the Study Vault.`}]);
      setTimeout(()=>{setVaultFile(null);setVaultCustomName("");setVaultProgress("");},2000);
    }
  ...
  async function sendChat(){
    ...
    if(activeStudy&&/(health|status|readiness|overview)/.test(lower)&&/(tmf|study|trial)/.test(lower)){
      setChatMessages(prev=>[...prev,{role:"ai",text:`${donePct}% complete, with ${missing} core documents outstanding and ${pending} awaiting review. Inspection readiness is ${ri}/100 for ${activeStudy.study_id}.`,isHealthCard:true,sourceTags:["Gap analysis","Inspection readiness","Document tracker"]}]);
  ...
  async function fileDocument(cl:any,validationResult:any){
    setChatLoading(true);
    try{
      const byteString=atob(cl.base64);const ab=new ArrayBuffer(byteString.length);const ia=new Uint8Array(ab);
  ...
  const TABS=[
    ...
    {id:"vault",label:`Study Vault${vaultDocs.length>0?` (${vaultDocs.length})`:""}`,Icon:IconDatabase},
    {id:"findings",label:`Findings${openFindings.length>0?` · ${openFindings.length} open`:""}`,Icon:IconAlert},
    {id:"briefing",label:"Daily Briefing",Icon:IconSun},
    {id:"checklist",label:`Study Checklist${checklist.length>0?` (${checklist.length})`:""}`,Icon:IconList},
  ];

  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"system-ui,-apple-system,sans-serif",background:P.bg}}>
      <style>{`@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}.ttab{border:none;background:transparent;cursor:pointer;padding:10px 16px;font-size:12px;font-weight:500;border-bottom:2px solid transparent;transition:all .2s;font-family:inherit;display:flex;align-items:center;gap:6px;}.ttab.active{border-bottom-color:#F97316;color:#F97316;}.ttab:not(.active){color:#6B7280;}.ttab:not(.active):hover{color:#374151;}`}</style>
      ...
      <div style={{display:"flex",alignItems:"center",borderBottom:`0.5px solid ${P.border}`,background:P.bg,paddingLeft:"1.5rem",flexShrink:0}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setPanel(t.id as any)} className={`ttab${panel===t.id?" active":""}`}>
            <t.Icon/>{t.label}
          </button>
        ))}
        <div style={{marginLeft:"auto",paddingRight:"1.5rem",display:"flex",gap:"8px",alignItems:"center"}}>
          {vaultDocs.length>0&&(
            <button onClick={generateChecklist} disabled={checklistLoading} style={{fontSize:"11px",padding:"5px 14px",background:P.cyan,color:"#fff",border:"none",borderRadius:"8px",cursor:checklistLoading?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:"6px",opacity:checklistLoading?0.7:1}}>
              {checklistLoading?<IconLoader/>:<IconList/>}
          ...
          {vaultDocs.length>0&&(
            <button onClick={runVaultAnalysis} disabled={analysing} style={{fontSize:"11px",padding:"5px 14px",background:P.purple,color:"#fff",border:"none",borderRadius:"8px",cursor:analysing?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:"6px",opacity:analysing?0.7:1}}>
              {analysing?<IconLoader/>:<IconBrain/>}
                  ...
                  <div style={{maxWidth:"78%",display:"flex",flexDirection:"column",gap:"6px"}}>
                    ...
                    {m.text?.startsWith("__FILED__")&&(
                      <div style={{display:"flex",alignItems:"flex-start",gap:"9px",border:"0.5px solid #bfe6d4",background:P.successLight,borderRadius:"10px",padding:"11px 14px"}}>
                        <span style={{color:P.success,flexShrink:0}}><IconCircleCheck/></span>
                        <div>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                          {m.validation.checks?.map((c:any,ci:number)=>(
                            <div key={ci} style={{display:"flex",alignItems:"flex-start",gap:"8px",padding:"7px 10px",borderRadius:"7px",background:c.pass?P.successLight:P.warningLight}}>
                              <span style={{flexShrink:0,color:c.pass?P.success:P.warning}}>{c.pass?<IconCheck/>:<IconAlert/>}</span>
                              <div style={{flex:1}}>
          ...
          <div style={{maxWidth:"800px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <button onClick={generateBriefing} disabled={briefingLoading} style={{fontSize:"12px",padding:"8px 16px",background:P.primary,color:"#fff",border:"none",borderRadius:"8px",cursor:"pointer",display:"flex",alignItems:"center",gap:"6px",opacity:briefingLoading?0.6:1}}>
                {briefingLoading?<IconLoader/>:<IconRefresh/>}
        ...
        <div style={{flex:1,overflowY:"auto",padding:"1.5rem"}}>
          <div style={{maxWidth:"900px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"16px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <button onClick={generateChecklist} disabled={checklistLoading||vaultDocs.length===0} style={{fontSize:"12px",padding:"8px 16px",background:P.cyan,color:"#fff",border:"none",borderRadius:"8px",cursor:"pointer",display:"flex",alignItems:"center",gap:"6px",opacity:checklistLoading||vaultDocs.length===0?0.5:1}}>
                {checklistLoading?<IconLoader/>:<IconRefresh/>}
            ...
            {!checklistGenerated&&vaultDocs.length>0&&(
              <div style={{textAlign:"center",padding:"3rem",color:P.textTert,background:P.bgSec,borderRadius:"12px",border:`0.5px solid ${P.border}`}}>
                ...
                <button onClick={generateChecklist} disabled={checklistLoading} style={{fontSize:"12px",fontWeight:"600",padding:"10px 20px",background:P.cyan,color:"#fff",border:"none",borderRadius:"8px",cursor:"pointer"}}>Generate Checklist</button>
              </div>
            )}
            ...
            {checklistGenerated&&checklist.length>0&&(
              <>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px"}}>
                  {[
                    {label:"Total expected",color:P.cyan,bg:P.cyanLight,count:checklist.length},
                ...
                <div style={{background:P.cyanLight,border:"0.5px solid #A5F3FC",borderRadius:"10px",padding:"12px 16px",fontSize:"11px",color:"#0E7490",display:"flex",alignItems:"center",gap:"8px"}}>
                  <IconBrain/>
                ...
                {["Critical","Major","Minor"].map(sev=>{
                  ...
                  const missingCount=sevItems.filter(c=>c.status==="Missing").length;
                  return(
                    <div key={sev}>
                      <h3 style={{fontSize:"11px",fontWeight:"600",color:SEVERITY_COLOR(sev),textTransform:"uppercase",letterSpacing:".06em",marginBottom:"6px"}}>{sev} — {missingCount} missing of {sevItems.length}</h3>
                      {sevItems.map((item,idx)=>(
                        <div key={idx} style={{background:P.bg,border:`0.5px solid ${item.status==="Missing"?SEVERITY_COLOR(item.severity):P.success}`,borderRadius:"10px",padding:"14px 16px",marginBottom:"6px",display:"flex",gap:"12px",alignItems:"flex-start"}}>
                          <div style={{width:"28px",height:"28px",borderRadius:"50%",background:item.status==="Filed"?P.successLight:SEVERITY_BG(item.severity),display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:item.status==="Filed"?P.success:SEVERITY_COLOR(item.severity)}}>
                            {item.status==="Filed"?<IconCheck/>:<IconAlert/>}
                          </div>
                          <div style={{flex:1}}>
                            <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px",flexWrap:"wrap"}}>
